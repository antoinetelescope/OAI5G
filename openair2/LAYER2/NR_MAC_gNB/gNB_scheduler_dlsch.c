                  ...
                  int n_rb_sched[num_beams])
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (sched_pdsch->dl_harq_pid >= 0) {
      ...
      LOG_I(NR_MAC, "[UE %04x][%4d.%2d], thr_ue %f\n",
            UE->rnti,
            frame,
            slot,
            UE->dl_thr_ue
...
static int compare_ue_coeff(const void *a, const void *b) {
    const UEsched_t *ue_a = (const UEsched_t *)a;
    const UEsched_t *ue_b = (const UEsched_t *)b;
    
    if (ue_a->coef > ue_b->coef) return -1;
    if (ue_a->coef < ue_b->coef) return 1;
    return 0;
}

static void wrr_dl(module_id_t module_id,
                  frame_t frame,
                  slot_t slot,
                  NR_UE_info_t **UE_list,
                  int max_num_ue,
                  ...
                  int n_rb_sched[num_beams])
{
    ...
    /* Phase 1: Collect eligible UEs and calculate coefficients */
    UE_iterator(UE_list, UE) {
        NR_UE_sched_ctrl_t *sched_ctrl = &UE->UE_sched_ctrl;
        NR_UE_DL_BWP_t *current_BWP = &UE->current_DL_BWP;
        ...
        if (sched_ctrl->ul_failure)
            continue;
        const NR_mac_dir_stats_t *stats = &UE->mac_stats.dl;
        /* Handle retransmissions first (unchanged) */
        NR_sched_pdsch_t *sched_pdsch = &sched_ctrl->sched_pdsch;
        sched_pdsch->dl_harq_pid = sched_ctrl->retrans_dl_harq.head;
        ...
        if (sched_pdsch->dl_harq_pid >= 0) {
            ...
            /* Calculate coefficient with priority */
            const NR_bler_options_t *bo = &mac->dl_bler;
            const int max_mcs_table = current_BWP->mcsTableIdx == 1 ? 27 : 28;
            const int max_mcs = min(sched_ctrl->dl_max_mcs, max_mcs_table);
	    ...
	    f1_ue_data_t ue_id = du_get_f1_ue_data(UE->rnti);
            int priority = ue_id.secondary_ue;
            LOG_I(MAC, "Priority : %d\n", priority);
    ...
    /* Phase 2: Sort UEs by weighted coefficient */
    qsort(UE_sched, curUE, sizeof(UEsched_t), compare_ue_coeff);

    /* Phase 3: Allocate resources in priority order */
    const int min_rbSize = 5;
    for (int i = 0; i < curUE; i++) {
        NR_UE_info_t *UE = UE_sched[i].UE;
        NR_UE_sched_ctrl_t *sched_ctrl = &UE->UE_sched_ctrl;
        const uint16_t rnti = UE->rnti;
        ...
        NR_beam_alloc_t beam = beam_allocation_procedure(&mac->beam_info, frame, slot, UE->UE_beam_index, slots_per_frame);
        if (beam.idx < 0 || remainUEs[beam.idx] == 0 || n_rb_sched[beam.idx] < min_rbSize) {
            reset_beam_status(&mac->beam_info, frame, slot, UE->UE_beam_index, slots_per_frame, beam.new_beam);
            continue;
        }

        /* Allocation logic remains the same but now in priority order */
        NR_sched_pdsch_t *sched_pdsch = &sched_ctrl->sched_pdsch;
        sched_pdsch->dl_harq_pid = sched_ctrl->available_dl_harq.head;
        sched_pdsch->time_domain_allocation = get_dl_tda(mac, slot);
        AssertFatal(sched_pdsch->time_domain_allocation>=0,"Invalid PDSCH time domain allocation\n");
        ...
        if (max_rbSize < min_rbSize) {
            LOG_D(NR_MAC, "(%d.%d) Cannot schedule RNTI %04x, rbStart %d, rbSize %d\n",
                  frame, slot, rnti, rbStart, max_rbSize);
            continue;
        }

        int CCEIndex = get_cce_index(mac, CC_id, slot, rnti, &sched_ctrl->aggregation_level,
                                   beam.idx, sched_ctrl->search_space, sched_ctrl->coreset,
                                   &sched_ctrl->sched_pdcch, false, sched_ctrl->pdcch_cl_adjust);
        if (CCEIndex < 0) {
            LOG_D(NR_MAC, "[UE %04x][%4d.%2d] could not find free CCE for DL DCI\n", rnti, frame, slot);
            reset_beam_status(&mac->beam_info, frame, slot, UE->UE_beam_index, slots_per_frame, beam.new_beam);
            continue;
        ...
        // Influence de la priorité attribuée : thr_ue multiplié par la priorité. (testé jusqu'à 3 UEs)

        f1_ue_data_t ue_id = du_get_f1_ue_data(UE->rnti);
        int priority = ue_id.secondary_ue;

        nr_find_nb_rb(sched_pdsch->Qm, sched_pdsch->R, 1, sched_pdsch->nrOfLayers,
                     tda_info->nrOfSymbols, sched_pdsch->dmrs_parms.N_PRB_DMRS * sched_pdsch->dmrs_parms.N_DMRS_SLOT,
                     (sched_ctrl->num_total_bytes  + oh) * priority, min_rbSize * priority, max_rbSize * priority, &TBS, &rbSize);
...
static void tbpf_dl(module_id_t module_id,
                  frame_t frame,
                  slot_t slot,
                  NR_UE_info_t **UE_list,
                  int max_num_ue,
...
{ 
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (sched_pdsch->dl_harq_pid >= 0) {
      ...
      // LOG_I(NR_MAC, "[UE %04x][%4d.%2d] b %d, thr %f, tbs %d, coef %f\n",
      //      UE->rnti, frame, slot, b, UE->dl_thr_ue, tbs, coeff_ue);

      LOG_I(NR_MAC, "[UE %04x][%4d.%2d] thr %f\n",
            UE->rnti, frame, slot, UE->dl_thr_ue);
  ...
  while (iterator->UE != NULL) {
    uint32_t token_threshold = 100;
    NR_UE_sched_ctrl_t *sched_ctrl = &iterator->UE->UE_sched_ctrl;
    f1_ue_data_t ue_id = du_get_f1_ue_data(iterator->UE->rnti);
    int ID = ue_id.secondary_ue;
    if (sched_ctrl->token_count == 0 && sched_ctrl->dl_bler_stats.mcs == 0) {
      sched_ctrl->token_count = 0;
      sched_ctrl->premium_subscriber = (ID % 2 == 0); // Pair = Premium
      LOG_I(NR_MAC, "[UE %04x, DL] Initialisation: premium=%d\n", iterator->UE->rnti, sched_ctrl->premium_subscriber);
    }
    ...
    LOG_D(NR_MAC, "[UE %04x, DL] Token count = %d (premium=%d)\n", iterator->UE->rnti, sched_ctrl->token_count, sched_ctrl->premium_subscriber);

    if (sched_ctrl->token_count < token_threshold){
      iterator++;
      continue;
    }
    ...
    if (sched_ctrl->token_count >= token_threshold && TokenBank[ID-1] == largest(TokenBank, 8)) {
      ...
      sched_pdsch->dl_harq_pid = sched_ctrl->available_dl_harq.head;
      sched_pdsch->time_domain_allocation = get_dl_tda(mac, slot);
      sched_pdsch->tda_info = get_dl_tda_info(&iterator->UE->current_DL_BWP,
                                              sched_ctrl->search_space->searchSpaceType->present,
                                              sched_pdsch->time_domain_allocation,
                                              scc->dmrs_TypeA_Position,
                                              1,
                                              ...
                                              false);
      if (!sched_pdsch->tda_info.valid_tda) {
        iterator++;
        continue;
      }

      const uint16_t slbitmap = SL_to_bitmap(sched_pdsch->tda_info.startSymbolIndex, sched_pdsch->tda_info.nrOfSymbols);
      uint16_t *rballoc_mask = mac->common_channels[CC_id].vrb_map[beam.idx];
      bwp_info_t bwp_info = get_pdsch_bwp_start_size(mac, iterator->UE);
      int rbStart = 0;
      int rbStop = bwp_info.bwpSize - 1;
      ...
      int CCEIndex = get_cce_index(mac, CC_id, slot, iterator->UE->rnti, &sched_ctrl->aggregation_level,
                                  beam.idx, sched_ctrl->search_space, sched_ctrl->coreset,
                                  &sched_ctrl->sched_pdcch, false, sched_ctrl->pdcch_cl_adjust);
      if (CCEIndex < 0) {
        reset_beam_status(&mac->beam_info, frame, slot, iterator->UE->UE_beam_index, slots_per_frame, beam.new_beam);
      ...
      int alloc = -1;
      if (!get_FeedbackDisabled(iterator->UE->sc_info.downlinkHARQ_FeedbackDisabled_r17, sched_pdsch->dl_harq_pid)) {
        int r_pucch = nr_get_pucch_resource(sched_ctrl->coreset, iterator->UE->current_UL_BWP.pucch_Config, CCEIndex);
        alloc = nr_acknack_scheduling(mac, iterator->UE, frame, slot, iterator->UE->UE_beam_index, r_pucch, 0);
        if (alloc < 0) {
          reset_beam_status(&mac->beam_info, frame, slot, iterator->UE->UE_beam_index, slots_per_frame, beam.new_beam);
          iterator++;
      ...
      sched_pdsch->dmrs_parms = get_dl_dmrs_params(scc, &iterator->UE->current_DL_BWP, &sched_pdsch->tda_info, sched_pdsch->nrOfLayers);
      sched_pdsch->Qm = nr_get_Qm_dl(sched_pdsch->mcs, iterator->UE->current_DL_BWP.mcsTableIdx);
      sched_pdsch->R = nr_get_code_rate_dl(sched_pdsch->mcs, iterator->UE->current_DL_BWP.mcsTableIdx);
      sched_pdsch->pucch_allocation = alloc;
      ...
      sched_ctrl->token_count -= token_threshold;
...
static void nr_dlsch_preprocessor(module_id_t module_id, frame_t frame, slot_t slot)
{
  ...
  if (get_softmodem_params()->enable_wrr == 1) {
    /* proportional fair scheduling algorithm */
    wrr_dl(module_id, frame, slot, UE_info->connected_ue_list, max_sched_ues, num_beams, n_rb_sched);
  } 
  else if (get_softmodem_params()->enable_tbpf == 1) {
    tbpf_dl(module_id, frame, slot, UE_info->connected_ue_list, max_sched_ues, num_beams, n_rb_sched);
  }
  else {
