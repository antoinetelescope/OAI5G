...
static void wrr_ul(module_id_t module_id,
                  frame_t frame,
                  int slot,
                  frame_t sched_frame,
                  int sched_slot,
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    LOG_D(NR_MAC,"wrr_ul: preparing UL scheduling for UE %04x\n",UE->rnti);
    NR_UE_UL_BWP_t *current_BWP = &UE->current_UL_BWP;
    ...
    * based on data to transmit) */
    ...
    float coeff_ue = (float) tbs * priority; 
  ...
  while (iterator->UE != NULL) {
    ...
    const int B = priority * cmax(sched_ctrl->estimated_ul_buffer - sched_ctrl->sched_ul_bytes, 0);
...
static void tbpf_ul(module_id_t module_id,
                  frame_t frame,
                  int slot,
                  frame_t sched_frame,
                  int sched_slot,
...
static bool nr_ulsch_preprocessor(module_id_t module_id, frame_t frame, slot_t slot)
{
  ...
  if (get_softmodem_params()->enable_wrr == 1) {
    ...
    wrr_ul(module_id, frame, slot, sched_frame, sched_slot, nr_mac->UE_info.connected_ue_list, max_sched_ues, num_beams, len);
  } 
  else if (get_softmodem_params()->enable_tbpf == 1) {
    /* token-based proportional fair scheduling algorithm */
    tbpf_ul(module_id, frame, slot, sched_frame, sched_slot, nr_mac->UE_info.connected_ue_list, max_sched_ues, num_beams, len);
  }
  else {
    /* proportional fair scheduling algorithm */
